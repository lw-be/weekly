...
* [190725]淘宝服务端高并发分布式架构演进之路[from 刘文健]

实践：

* [190721]8种你可能正在写错的SQL用法[from 大瑞]
* [190329]为什么阿里禁止在 foreach 循环里进行元素的 remove/add 操作[from 曹伟伟]
* [190721]阅读源码的利器 — Intellij IDEA中 Replace in Path 的使用技巧[from 车谦波]
* [190724]推荐一款 MyBatis 开发神器，为简化而生！[from 翟旭]
* [190726]负载均衡算法[from 陈强]
* [190727]Arthas工具介绍[from 李广]
* [190720 ]springboot自定义starter[from 唐剑飞]
* [20190701]结巴分词 (from 小瑞)
    * 主要内容
    * 特点
    * 安装说明
    * 算法
...
资讯：
* [ 190317 ]中台，数字时代企业需要掌握的新技能[from 李广]
* [ 190720 ]【数据结构与算法】动画：什么是 BF 算法 ？[from 车谦波]

实践
* [ 190716 ]Java 代码界 3% 的王者？看我是如何解错这 5 道题的[from 大瑞]
* [ 190717 ]Java 8 创建 Stream 的 10 种方式，我保证你受益无穷![from 大瑞]
* [ 140911 ]Java 8 中的 Streams API 详解[from 俊路]
    * 主要是对java8 中stream的介绍与相关api的讲解
* [ 190719 ]全文搜索引擎 Elasticsearch[from 车谦波]
* [ 180208 ]从Elasticsearch来看分布式系统架构设计[from 小瑞]
* [ 190715 ]浅谈正则表达式原理[from 杜文刚]
* [ 190720 ]lombock使用 [from 陈强]
* [ 190720 ]花了一个星期,我终于把RPC框架整明白了![ from 翟旭]
* [ 190720 ]springboot自定义starter[from 唐剑飞]
* [ 190720 ]跳表：为什么Redis一定要用跳表来实现有序集合？[from 曹伟伟]
* [ 190628 ]jersey [from 曹永红]
...
资讯：
* [ 170817 ]全文搜索引擎 Elasticsearch[from 小张瑞]
* [ 190713 ] Elasticsearch权威指南（中文版）[from 小张瑞]
* [ 190713 ] 可视化算法库[from 大瑞哥]
    * 提供c++,java,js的解析
    * 可以用js,java,c++来写算法
* [ 190712 ] 工作中学习快捷键系列[from 曹永红]
    * 简单记录自己在工作中遇到的有趣快捷键，不会的快捷键，遗忘的快捷键
    * 包括idea、linux、google浏览器、markdown的快捷键使用
    * 文章记录的包括快捷键但又不限于快捷键
* [ 190713 ] Google Guava官方教程（中文版）[from 杜文刚]
    ...
    * 数学运算[Math]
    * 反射[Reflection]
* [ 190713 ] Google Guava User Guide（英文版）[from 杜文刚]
* [ 190709 ] 一般电商应用的订单队列架构思想[from 杜文刚]
    * 一般的订单流程
    * 思考瓶颈点
    * 订单队列
    * 实现队列的选择
    * 解答
    * 第二种队列的 Go 版本例子代码
* [ 190712 ] 漏洞预警！Fastjson远程代码执行漏洞（哨兵云支持检测） 临时处置方法：升级 Fastjson 到 1.2.48 及以上版本[from 胡大民]
* [ 190711 ] 5G相关介绍PPT，98页[from 胡大民]
* [ 190713 ] 心情不好时，看看这3张图，瞬间豁然开朗[from 胡大民]
* [ 190713 ] feed流-推[from 路璐]
* [ 190713 ] feed流-拉[from 路璐]
* [ 171025 ] 聊聊分布式事务，再说说解决方案[from 杨俊路]
    * 主要介绍了：
    * 分布式事务 CAP定理
    * 分布式事务的几种解决方案的介绍
    * 两阶段提交（2PC）
...
资讯：
* [190704]掘地三尺，我挖出了这次线上事故的元凶：Java内存泄漏！[from 大瑞哥]
* [190705]Hello Redis，我有7个问题想请教你！[from 大瑞哥]
* [190704]Ebean 的事务介绍[from 杨俊路]
    * 主要是讲解了Ebean的事务如何应用
    * 1、 事务的开启
    * 2、 事务的边界
    * 3、 事务中batch的处理等
* [190701]VIM常用命令大全[from 路路]
* [170820]Google Guava Cache 全解析 [from 小张瑞]
* 技术圈-大数据/算法[from 唐剑飞]
* 看大佬玩转numpy[from 唐剑飞]
* python-numpy入门教程[from 唐剑飞]
...
* [190415]腾讯重磅开源 DCache，分布式 NoSQL 存储系统 热[from 杨俊路]
    * 以问答的方式介绍了腾讯 DCache 创建的原因，架构图，主要是对DCache有一个粗略的了解
* [190606]RPC的负载均衡策略[from 杨俊路]
    * 本文主要图解了几种对rpc请求的负载均衡策略（未讲具体实现） 主要是图解了几种负载均衡策略
    * 代理服务
    * 客户端负载均衡
    * Lookaside 负载均衡
* [170711] you-get轻便的视频/图片/音频下载工具 [from 唐剑飞] 还在为视频下载而苦恼么？you-get 一句话搞定~
* [180920] ffmpeg 视频处理好帮手 [from 唐剑飞] 强大的视频/音频处理工具。视频剪辑、压缩...效率高、损失低。
* [180216] OpenCV-Python 中文教程 [from 唐剑飞] OpenCV-Python的中文教程。
* [180525]分布式架构的演进过程 [from 曹永红]
    * 分布式架构常见概念：集群、分布式、节点、副本机制、中间件
    * 图文并茂，以炒菜和商品交易系统描述了分布式架构出现的原因
    * 分8个阶段讲解了分布式架构发展过程
    * 集群不等于分布式，分布式架构可能包含集群。1、集群解决访问量大和数据备份问题。 2、分布式解决访问量大的问题和避免系统整体瘫痪。
* [180619]浅谈服务治理与微服务 [from 曹永红]
    * 本篇文章先简单介绍了互联网架构的演变，进而介绍了服务化，最后再介绍微服务
    * 一体架构>mvc架构>多应用架构>分布式架构>微服务
    * 微服务有两个核心：
    * 微：服务的粒度要细，即服务要细化到API
    * 服务：提供好服务，要让用户感到好用
* [190509] 深度 | API 设计最佳实践的思考 [from 张瑞（大）]
* [180312] 影响MySQL查询速度慢与性能差的因素[from 杜文刚]
    * 主要介绍了：
    * 什么影响了数据库查询速度
    * 什么影响了MySQL性能
* [ 190624 ] 终于有人把中台说清楚了[from 胡大民]
    * 所谓的“中台”，并不是阿里巴巴首先提出的词语，从字面意思上理解，中台是基于前台和后台之间。阿里通过多年不懈的努力，在业务的不断催化滋养下，将自己的技术和业务能力沉淀出一套综合能力平台，具备了对于前台业务变化及创新的快速响应能力。阿里人将“中台战略”形象地比喻成陆海空三军立体化协同作战。他们将中台分为6类：
        ...
        * 组织中台，为项目提供投资管理、风险管理、资源调度等，是战场的指挥部，战争的大脑，指挥前线，调度后方。 中台就是公共服务平台。
...
实践
* [190620] 一份详尽的 Java 问题排查工具清单，值得收藏！ [from 小张瑞]
* [190626] MySQL全面优化，速度飞起来！ [from 小张瑞]
* [151106] Thrift实战 教我们如何通过Thrift构建一个RPC服务以及语法简介 [from 路璐]
    * 参考1 包括一个demo，协议，传输层，服务构建等方法的介绍
    * 参考2 RPC服务治理、zookeeper、thrift服务提供者设计思路
    * 参考3 一个Demo
...
* [190621] MySQL缓冲池介绍 主要讲解了MySQL缓冲池的具体实现方案 [from 路璐]
* [181221] 服务端高并发分布式架构演进之路 [from 张瑞（大）]
* [190316] Java高级开发必会的50个性能优化的细节 [from 杨俊路] 主要针对的是一些java代码中的细节，针对一些 关键字、对象、数组、列表、Map、线程等进行了简单的描述，在实际工作中比较有用，有些优化在dispach中有应用
    * 以下是几个比较典型的优化
    * 尽量使用移位来代替'a / b'，'a * b'的操作
    * 尽量早释放无用对象的引用(dispath中有使用)
    * 尽量使用final修饰符，让访问实例内变量的getter/setter方法变成”final”
    * 尽量避免使用split（dispath中有使用）
    * 尽量合理的创建HashMap，合理使用HashMap(int initialCapacity, float loadFactor)这个构造方法（dispath中有使用）
    * 使用System.arraycopy ()代替通过来循环复制数组
* [170722] kubernetes安全控制认证与授权 认证与授权过程需使用HTTPS [from 胡大民]

实践

* [ 171024 ] Java高性能反射工具包ReflectASM [from 唐剑飞]
    * 相关： 1、java的ASM技术 2、使用过程中的注意点，个人demo演示，适用场景
* [190618] 自定义注解工具原理、实现、应用 (from 张瑞（小）)
...
资讯：
* [181221]解读2018：我们处在一个什么样的技术浪潮当中？[from 永红]
...
* [20180111]Service Mesh深度解析[from 路璐]
    * 文中主要以一下三点展开分析的Service Mesh
    * Service Mesh概念
    * Service Mesh的演进历程
    * 选择Service Mesh的原因
* [ 190613 ] Vim 和 NeoVim 曝出高危漏洞[from 胡大民] Vim 8.1.1365 和 Neovim 0.3.6 之前的版本都受到影响，文章演示了如何通过漏洞控制你的计算机
* [190615] 码小辫 公众号：imaxiaobian [from 张瑞（大）]
    * 这个公众号书挺多的，而且支持发送 java spring 关键字，会回复书列表和下载链接

实践

* [2018.01.01]1、图解 kafka 的高可用机制\ 2、帮助理解[from 杨俊路]
...
* [ 190605 ] 命令行的艺术 https://github.com/jlevy/the-art-of-command-line/blob/master/README-zh.md Star 5W+的文章，翻译成了多国语言。 [from 胡大民]
* [190614] dispatch的帮助工程 readme有介绍[from 张瑞（大）]
...
* [190425]mysql 8.0.16新版介绍 [from 永红]
    ...
    * linux
* [140101] [Pro Git book] (https://git-scm.com/book/zh/v2) [from 张瑞] BitKeeper和Linux合作关系结束，然后Linus开发了git
    * 读书笔记:
...
* [ 160102 ] 一张图看尽 Linux 内核运行原理 [胡大民] 在 Linux 内核中，有许多层次、模块、功能调用和函数，本文提供一个非常酷的完整的内核交互图，上面非常清晰地标注了每一个组件部分及之间的关系。该交互图可以帮助你不用一一细读 Linux 内核代码，就能弄明白各个内核子系统间复杂的内部联系。图上包含有超过400个关键函数，这400个关键函数被分进了一些主要的子系统，彼此之前的关系用连线标明了 linux
* [ 190102 ] google TF教程 [ from 剑飞 ]
    ...
    * ....
    * 3、三个实际案例
* [180206] 详解JVM Garbage First(G1) 垃圾收集器 [from 俊路]
    * 读书笔记： 1、简单讲解了一下几种垃圾回收机制的组合串行、并行、并发标记清除收集器
    * 2、详细讲解了G1的内存模型
    * 分区概念
    * 分代模型
    * 分区模型
...
资讯
* [ 180502 ] python WordCloud 简单实例 [ from 剑飞 ]

实践

* [ 190524 ] 用故事来给你讲负载均衡的原理 [ from 大瑞 ]
